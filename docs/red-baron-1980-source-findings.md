...
can audit citations against the quarry.
...
**⚠ THE RADIX RULE — stated once, and only here.** Every numeric constant in this document carries
its radix explicitly (`0x18`, or a `.`-suffixed decimal). Read a constant without checking its
region and you will be wrong by up to 16×, which is the defect rb4-1 swept out of the whole codebase.

| Region | Radix | Boundary |
|---|---|---|
| `RBARON.MAC:74` – `:6216` | **hex** | `.RADIX 16` at `RBARON.MAC:74` |
| `RBARON.MAC:6217` – `:6280` | **decimal** — the lone island (the vertex/`POINTP` tables) | `.RADIX 10` at `RBARON.MAC:6217` |
| `RBARON.MAC:6281` – EOF | **hex** again | `.RADIX 16` at `RBARON.MAC:6281` |

Those are the *only* three `.RADIX` directives in the file. Inside a hex region a **trailing dot**
forces decimal for that literal alone (`CMP I,250.` is 250, not `0x250`=592) — and the picture ROM
`037007.XXX` is `.RADIX 10` throughout, which is why `topology.ts`'s decimal transcription is right
and must not be "corrected".

**⚠ THE LINE NUMBERS.** Citations here are counted against the **LF byte-of-record** of the quarry
(`RBARON.MAC` = 6294 lines; `:74` = `.RADIX 16`; `:621` = `CALCNT =18`). A CRLF checkout of the same
source glues each form-feed page break onto the following `.SBTTL` line and is **8 lines short**, with
the shortfall accruing in a staircase (+1 by `:621`, +8 by the vertex table). Resolve a citation
against that copy and it silently points at the wrong line — which is exactly how rb1-2's citations
came out stale. There is **no constant offset** that converts between the two.

<!-- decoy-trap:start -->
> ## ⚠ THE DECOY BUILD — READ THIS BEFORE YOU CITE ANYTHING
>
> **The quarry contains a build that never shipped, and it is designed to fool you.** This document
> was written against it once already (rb1-2), and every fact it sourced there had to be retracted
> (rb4-2). **Nothing below this header may cite `R2BRON` or `R2GRND`.** The shipped build — the only
> citable one — is `RBARON`/`RBGRND` (+ `RBSOUN`, `RBCOIN`, `RBINT`, `RBROM`, `VGUT`, `TCN65`,
> `VGMC`, and the picture/char ROMs `037007.XXX`/`037006.XXX`).
>
> **Why the decoy is so convincing.** `R2BRON.MAC` is byte-identical to `RBARON.MAC` except for
> **7 lines — and every one of them is a ROM self-test checksum byte** (`CHKSM2`…`CHKSM7`, zeroed in
> the decoy). Its *code and data are the shipped code and data*. So a citation to `R2BRON.MAC:NNNN`
> lands on the right text at the right line and appears to verify. It sails through review. Thirty-one
> of them did.
>
> **Where the lie actually enters.** `R2BRON`'s own load map links **`R2GRND`**, not `RBGRND` — and
> those two differ in exactly **TWO** lines, both of which matter:
>
> | | shipped `RBGRND.MAC` | decoy `R2GRND.MAC` | consequence |
> |---|---|---|---|
> | `:61` | `FRMECNT=4` | `FRMECNT=5` | VG refresh **62.5 Hz**, not 50 Hz |
> | `:197` | `CMP I,3` | `CMP I,40` | watchdog trips at **`CALFLG ≥ 3`**, not `0x40` (=64) — off by 21× |
>
> An earlier draft of this header claimed the two builds differed in the frame divider *alone*. They
> do not. That sentence is how the `0x40` watchdog — a value that exists only in the build that never
> ran — got into this document and into `src/`.
>
> **And the decoy signs the ship build's name.** `R2BRON.MAP` (the load map) identifies its own object
> module as `RBARON`:
>
> ```
> OBJ:R2BRON      RBARON          10-SEP-81  15:19:36
> OBJ:R2GRND      RBGRND          10-SEP-81  15:23:25
> ```
>
> So the decoy's build artefacts *call themselves* `RBARON`/`RBGRND`. Identity by filename is worthless
> here; go by the LINK recipe. **`RBARON.COM`** links `RBARON,RBCOIN,RBSOUN,RBGRND,VGUT,RBROM,RBINT`
> and emits the released part images `037001.01, 037000.01, 036999.01, …`, matching the `RBARON.DOC`
> part manifest. That is the machine in the cabinet. (The `RBARON.DOC` "MAIN PROGRAM SOURCE FILES"
> note that lists the `R2` files is a stale manifest line, contradicted by the build recipes and the
> release part numbers.) [ROM-verified]
<!-- decoy-trap:end -->
...
- **Game-logic / calculation frame = NMI ÷ `CALCNT`.** `C.CNTR` counts NMIs; at zero reloads
  ...
  ~10.42 Hz** (`CALCNT`, `RBARON.MAC:621`).
  Attract/banner **`BNRCNT=0x0C=12` → 48 ms → ~20.83 Hz** (`BNRCNT`, `RBARON.MAC:622`).
  [ROM-verified]

The main loop (`MAIN`, `RBARON.MAC:763`) computes the **entire** sim + display list once per
calculation frame, waits on `CALFLG` then `INTFLG` (`INTWAIT`, `RBARON.MAC:926-933`), swaps
buffers, restarts the VG, and `INC FRAME` (`RBARON.MAC:870`). While it computes, the NMI keeps
...
| `FRAME` | **game-logic frame counter** (the ÷N masks key off this) | 1× per calc-frame | `RBARON.MAC:140`; `RBARON.MAC:870` |
...
| **Game-logic / calc frame (`CALCNT`)** | ÷24 NMIs | **96 ms / ~10.42 Hz** | `RBARON.MAC:621`; `RBGRND.MAC:221-229` |
| Calc frame, attract (`BNRCNT`) | ÷12 NMIs | 48 ms / ~20.83 Hz | `RBARON.MAC:622` |
| Enemy/ground shell launch (`SHLAUN`) | ÷4 calc-frames (`FRAME&3`) | ~384 ms | `RBARON.MAC:4027-4030` |
| Plane fires shells (`PLNSHL`) | ÷2 calc-frames (`FRAME&1`) | ~192 ms | `RBARON.MAC:4809-4812` |
| Player shell sub-stepping (`SHLMOT`) | **4×** per calc-frame | — | `RBARON.MAC:5192-5204` |
| Shot-sound modulation | ÷8 NMIs (`INTCNT&8`) | 32 ms half-cycle | `RBGRND.MAC:171-174` |
| POKEY self-check (`PKYCNT`) | ÷10 calc-frames | ~960 ms | `RBARON.MAC:849-850` |
...
period — if 6502 compute overran it the loop slowed. The watchdog trips once **`CALFLG ≥ 3`** —
`LDA CALFLG` / `CMP I,3` / `BCS`, commented `;3*CALCULATION LOOP TIME W/O FRAME`
(`RBGRND.MAC:196-202`) — i.e. after **three** missed calculation frames, not sixty-four. (An earlier
draft of this document read that threshold off the decoy's ground module, whose `:197` says
`CMP I,40` = `0x40` = 64 instead. It was wrong by 21×. See the decoy header.) Treat 96 ms as the fixed
timestep unless MAME playtest says otherwise. [ROM-verified constants; wall-clock rate inferred]

---

## 2 · Player flight model (seeds rb1-3)

**Source:** `RBARON.MAC` (`POT.X`, `POTSCL`, `PFMOTN`, `DISCHK`, `PFTRIG`); `RBGRND.MAC` (`NMI`,
...
D6=start (`RBARON.MAC:521`). A single pot line `POTIN` is multiplexed between the two yoke axes by
the **POT SELECT** bit (`PS.BIT`, `CRSHSN=$1808` D0); the NMI (`RBGRND.MAC:104-161`) alternates
...
(`RBARON.MAC:88-167`): `UNIV4X` (universe X ×4), `I4YPOS` (eye/horizon Y ×4), `PFROTN` (horizon
roll angle), `PFXSCR` (horizon X scroll). [ROM-verified]

- **Turn / roll → `PLDELX`, a rate with inertia.** `POT.X` (`RBARON.MAC:5897-5926`) eases `PLDELX`
...
- **Pitch / climb-dive → `PLDELY`, 11 discrete steps.** `POTSCL` (`RBARON.MAC:5838-5871`) maps the
  pitch pot to index 0–10 into `POTDLY: .4WORD -32,-23,-17,-10,-5,0,4,8,13,18,25`
  (`RBARON.MAC:5930`). Center = 0; **asymmetric — dive (−32) is faster than climb (+25)**. [ROM-verified]
- **`PFMOTN` — "update center of screen" (the flight-camera pipeline)** (`RBARON.MAC:3154-3267`):
  ...
  - **Altitude:** `PLDELY` (×`DISCHK`) adds to `I4YPOS`, **hard-clamped `PLYMIN=8*4 … PLYMAX=180*4`**
    (`RBARON.MAC:446-456`). Because altitude is clamped, you can't crash by pitching into the ground
    in a normal dogfight — terrain only bites in the ground wave (§4). [ROM-verified]
- **Distance-scaled control feel (`DISCHK`, `RBARON.MAC:3468-3496`):** player deltas scale by
...
- **Rotation math (`PFTRIG`/`TRIG`, `RBARON.MAC:5981-6053`):** turns `PFROTN` (bit format
  `XXX XQQA AAAA AA.FF` = quadrant+angle+fraction) into sine/cosine → Math Box matrix regs
  `MM.A`(cos)/`MM.B`(sin). Projector `POSITP`/`POSITH` (`RBGRND.MAC:261-322`) translates by
...
**Source:** `RBARON.MAC` (`NWPLNE`/`STPLNE`, `UPDPLN`, `PLNDEL`/`P.WINDW`, `P.UPD0`, `PLNSHL`,
`UPPLEX`, `BLMOTN`).

- **Object budget: 3 motion objects = 1 lead plane + 2 "drones" (wingmen)**; a blimp can borrow a
  slot. `N.MOB` "3 MOTION OBJECTS (1 PLANE, 2 DRONES)" (`RBARON.MAC:462-463`). Per-object status
  byte `A E C R @ ED P F`: D7 active, D6 explosion, **D3 shoots**, D1/D0 drone flags
  (`RBARON.MAC:274-276`). [ROM-verified]
- **Spawn cadence & count scale with score** (`NWPLNE`/`STPLNE`, `RBARON.MAC:2241-2390`): enemy
...
- **Steering AI is a weaving window-follower, NOT a beeline seeker** (`UPDPLN`/`PLNDEL`/`P.WINDW`,
  `RBARON.MAC:2570-2874`): the plane accelerates its ΔX (`ACCEL=30`) toward window limits and
  **reverses at inner/outer boundaries**, weaving across screen center; it **banks proportional to
  ...
  `GMLEVL`-indexed (`P.OLIM 40,80,120,1A0,200` / `P.ILIM 20,30,80,120,160`, `RBARON.MAC:2939-2956`)
...
- **The "Red Baron" pass: fly-by → returning attack from behind** (`P.UPD0`,
  `RBARON.MAC:2727-2742`): when a plane closes past `P.MNDP=140` it enables returning-plane shells,
...
- **Firing: only aggressive/high-level planes shoot, every other frame** (`PLNSHL`,
  `RBARON.MAC:4804-4813`): fires only if status D3 set; `FRAME` LSB gates ÷2. The D3 "@ PLAYER" bit
...
- **Killed enemy = falling/spinning wreck; a wingman is promoted to lead** (`UPPLEX`,
  `RBARON.MAC:2961-3034`): gravity `EX.ACY=-20` per frame, spins about Z; `.EXPL1=6` frames fall →
...
- **Blimp/Zeppelin** (`BLMOTN`, `RBARON.MAC:4170+`): ~25 % random spawn, drifts across, **also fires
...
**Source:** `RBARON.MAC` (`NWPLNE`/`MODECT`, `INITGR`, `PFOBMN`, `GRDISP`); `RBGRND.MAC`
(`SCAPE0..3`, `PFOCOL`).

- **Play alternates PLANE waves and GROUND (strafing) waves.** A `NEWCT` countdown steps `MODECT`,
  whose LSB selects plane wave (`STPLNE`) vs ground wave (`INITGR`) (`RBARON.MAC:2258-2273`).
  Inter-wave counts `MCOUNT: .BYTE 4,2,3,2,1,3,4,2` (`RBARON.MAC:1298-1299`). `.LEVLS=5` is the
  difficulty *ceiling* reached via kills (§3), not discrete stages. [ROM-verified]
- **Ground wave = low-altitude run over a scrolling landscape** (`INITGR`, `RBARON.MAC:1403-1409`):
  sets `GRMODE=0C0` (D7 ground + D6 plane-disable), so the main loop skips new-plane generation and
  slows control. Landscape = four mountain-range silhouettes `SCAPE0..SCAPE3` (`RBGRND.MAC:725-798`,
...
- **Ground targets** displayed by `GRDISP` (`RBARON.MAC:3555-3711`) using `PFOCOL` collision
...
- **Scoring tied to mechanics** (`PLNSCR`/`DRNSCR`, `RBARON.MAC:3038-3050`): a lit/close plane scores
...
**Source:** `RBARON.MAC` (`NEWSHL`/`SETSHL`, `CDSSET`/`SHCDCK`, `EOLSEQ`/`ENDLFE`, `GMINIT`/`INITIAL`).
...
- **Player shells vs enemies (air):** fire while held, **alternating L/R guns**, with a **gun-overheat
  ...
  warning (`NEWSHL`/`NWSHL1`, `RBARON.MAC:2153-2237`). 13 shell slots; shells advance in Z, expire at
...
- **Enemy fire vs player is NOT a per-pixel hit test.** Two distinct death channels:
  - **(a) The returning ace — an evade check, not a projectile collision** (`EOLSEQ`,
    `RBARON.MAC:1072-1104`): during the ace's pass, at `PLSTAT+7==0C` the game checks the player's
  ...
  - **(b) The ground:** *ground fire* (`SHLAUN`, `RBARON.MAC:4027-4115`) leads/aims at the player,
    ...
    main loop aborts the frame ("PLAYER RAN INTO GROUND", `RBARON.MAC:783-784`). [ROM-verified]
- **Lives & respawn** (`EOLSEQ→ENDLFE`, `RBARON.MAC:1057-1212`): on death the windshield **bullet-hole**
...
- **Respawn spawn-grace** (`GMINIT`/`INITIAL`, `RBARON.MAC:1217-1293`): on (re)spawn,
...
|---|---|---|---|---|
| Score tick (small) | `TK` | `SOUND 0` (`RBARON.MAC:1577`) | ch1 | one-shot, fades |
| Score tick (larger) | `TP` | `SOUND 4` (`RBARON.MAC:1580`) | ch1 | one-shot, fades |
| Bonus life | `BN` | `SOUND 2` (`RBARON.MAC:1596`) | ch1 | rising warble ×6 |
| Enemy plane announce | `WP` | `SOUND 1` (`RBARON.MAC:2317,3017`) | ch3 | descending ×3 |
| 300-point jingle | `TH` | `SOUND 3` (`RBARON.MAC:3042`) | ch2 | 6-note melody |
...
oscillators `$F8/$F7` on ch3/ch4 (`RBARON.MAC:1039-1042`, `AUDC=$A1`); an **enemy-approach whine**
ramps `ATGVAL` on ch3/ch4 by distance (`RBARON.MAC:1001-1037,2447-2460`). The analog *timbre* (noise
...
signed 8-bit coordinate (`RBARON.MAC:4879-4881`: `DB.PLN = Z (S7654321), +1 = X, +2 = Y`). `.4WORD/.3WORD/.2WORD`
...
(`RBARON.MAC:6233`): `POINTP -40,20,-40 ;12 TOP WING` → model coord **(X=−40, Y=+20, Z=−40)** = the left tip
of the upper wing. [ROM-verified]

**Biplane 3-D model ("PLANE POINTS DB", `RBARON.MAC:6215-6287`):** 42 vertices total. There are two
point-sets: the *full* plane = 42 pts (`.PLPNT`), and the reduced plane = **29 pts** (`.DRPNT`, points
0-28 only, no back faces). The full model draws all 42 + back-face list `DB.MAP` + wing struts
`DB.LNS`; the reduced one draws 29 + front list `DB.MAR`. `BLCOLL` = the 8-vertex ±16,±16,±40 collision
cube (hit tests, not display). Root pointer table `PLNDB` (`@ 709E`) indexes every drawable point-set
(plane, prop, collision box, explosion pieces, blimp, stars). [ROM-verified vertices & counts]

> **RETRACTED (rb4-2) — the model is NOT selected by distance.** An earlier draft of this section
> described the two point-sets as a depth-driven level-of-detail switch (the *drone / distant*
> plane), tagged `[LOD reading inferred]` — it was a guess, and it was wrong. `biplane.ts` then grew
> a `LOD_DISTANCE` constant to implement a rule the ROM does not have.
>
> **What the ROM actually does.** `DRNPIC` (`RBARON.MAC:4961`) selects the model on an **orientation
> bit**, not a depth compare:
>
> ```
> DRNPIC: LDA PLSTAT+6            ;PLANE ROTATED
>         AND I,10                ; <- 0x10 (D4). hex region: NOT decimal 10.
>         BEQ 20$                 ;YES -> .DRPNT, the 29-point set + DB.MAR
> ```
>
> The bit is `PLSTAT+6` **D4 = `0x10`**, set/cleared at `RBARON.MAC:2652` (`;D4=0 (PLANE FACING AWAY)`).
> Bit clear → the plane is turned away → draw the reduced 29-point set. Bit set → draw the full model,
> scaled. **No distance test exists anywhere in the picture path**: a plane one unit away that is facing
> away draws the 29-point set, and a plane at maximum range that is facing you draws all 42.
> Retiring `biplane.ts`'s `LOD_DISTANCE` in favour of the orientation bit is **rb4-13**. [ROM-verified]
...
**Picture-ROM object inventory** (addresses from `RBARON.MAC:379-438` equate chains, anchored by
...
- **2026-07-09 — rb1-2 (initial).** Quarried `github.com/historicalsource/red-baron` into the gitignored
  ...
  62.5 Hz vs 50 Hz — was adjudicated against the two builds' `.COM` LINK recipes and the release part
  numbers: **canonical = `RBARON`/`RBGRND`, `FRMECNT=4`, 62.5 Hz** (see the decoy header). The committed doc is
  self-contained; the `reference/` quarry it was pulled from is gitignored and not required for any fact here.
- **2026-07-14 — rb4-2 (retraction).** The rb1-2 draft cited the decoy build and counted its lines against a
  CRLF checkout, so its enemy facts came from a program that never ran and its line numbers were stale by
  1–8. Every citation re-resolved against the LF byte-of-record; the decoy quarantined behind the header
  above; the watchdog threshold corrected from `0x40` to `3`; the depth-driven model-select claim
  retracted (§7 — the ROM switches on an orientation bit).
